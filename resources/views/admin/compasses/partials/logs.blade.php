<div class="col-sm-3 col-md-3 sidebar">
  <h3><i class="voyager-logbook"></i> 日志
    <small>应用程序日志</small>
  </h3>
  <div class="list-group">
    @foreach($files as $file)
      <a href="?logs={{ base64_encode($file) }}"
         class="list-group-item @if ($filename === $file) llv-active @endif">
        <i class="voyager-file-text"></i> {{ $file }}
      </a>
    @endforeach
  </div>
</div>
<div class="col-sm-9 col-md-9 table-container">
  @if ($logs === null)
    <div>日志文件大于 50 MB，请去下载！</div>
  @else
    <table id="table-log" class="table table-striped table-condensed">
      <thead>
      <tr>
        <th>级别</th>
        <th>环境</th>
        <th>时间</th>
        <th>内容</th>
      </tr>
      </thead>
      <tbody>

      @foreach($logs as $key => $log)
        <tr data-display="stack{{{ $key }}}">
          <td class="text-{{{ $log['level_class'] }}} level">
            <span class="glyphicon glyphicon-{{{ $log['level_img'] }}}-sign" aria-hidden="true"></span>
            &nbsp;{{ $log['level'] }}
          </td>
          <td class="text">{{ $log['context'] }}</td>
          <td class="date">{{{ $log['date'] }}}</td>
          <td class="text">
            @if($log['stack'])
              <a class="pull-right expand btn btn-default btn-xs" data-display="stack{{{ $key }}}">
                <span class="glyphicon glyphicon-search"></span>
              </a>
            @endif
            {{{ $log['text'] }}}
            @if(isset($log['in_file'])) <br/>{{{ $log['in_file'] }}}@endif
            @if($log['stack'])
              <div class="stack" id="stack{{{$key}}}" style="display:none;white-space:pre-wrap;">
                {{{ trim($log['stack']) }}}
              </div>
            @endif
          </td>
        </tr>
      @endforeach

      </tbody>
    </table>
  @endif
  <div>
    @if($filename)
      <a href="?download={{ base64_encode($filename) }}">
        <span class="glyphicon glyphicon-download-alt"></span> 下载文件
      </a>
      -
      <a id="delete-log" href="?delete={{ base64_encode($filename) }}">
        <span class="glyphicon glyphicon-trash"></span> 删除文件
      </a>
      @if(count($files) > 1)
        -
        <a id="delete-all-log" href="?remove=true">
          <span class="glyphicon glyphicon-trash"></span> 下载全部文件
        </a>
      @endif
    @endif
  </div>
</div>
